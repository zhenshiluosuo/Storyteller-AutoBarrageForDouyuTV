...
(function () {
    ...
    let css1 = 'background: #1A59B7;color:#ffffff;overflow: hidden;z-index: 998;position: fixed;padding:5px;text-align:center;width: 75px;height: 22px;border-bottom-left-radius: 4px;border-bottom-right-radius: 4px;border-top-left-radius: 4px;border-top-right-radius: 4px;right: 10px;top: 72%;'
            ...
            if(document.getElementsByClassName('danmu-6e95c1')[0].childNodes.length){
                ...
                div_manmu.addEventListener('DOMNodeInserted', function () {
                    if(!_mode){//如果是套娃模式 不需要找句子
                        ...
                        sentence = '@' + div_wenzi.childNodes[div_wenzi.childNodes.length - 1].getElementsByClassName('Barrage-content')[0].innerText + div_wenzi.childNodes[div_wenzi.childNodes.length - 1].getElementsByClassName('Barrage-nickName Barrage-nickName--blue js-nick')[0].innerText;
                        _ready = true;
                    }
                },false);
